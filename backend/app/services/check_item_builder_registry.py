...
# ============================================================================
# Phase 3.3A — Fact Collection Item Builders
# ============================================================================


class _OsBasicFactCollectionBuilder(BaseCheckItemBuilder):
    """Generate items for OS_BASIC_FACT_COLLECTION check.

    For each server asset, resolves OS credential and builds one item.
    """
    ...
    def build(self, db, *, assets, options):
        ...
        for asset in assets:
            ...
            credential = CredentialResolverService.resolve_os_credential(db, server_id)

            item_key = f"server:{server_id}:OS_BASIC_FACT_COLLECTION:{target_host}"
            item = {
                "item_key": item_key,
                ...
                "is_required": False,
                "timeout_seconds": timeout_seconds,
                "asset_name": server.hostname or server.server_code or f"server-{server_id}",
                "os_family": "linux",
            }

            if credential:
                item["credential_profile_id"] = credential["credential_profile_id"]
                item["credential_code"] = credential["credential_code"]
                item["credential_role"] = credential["binding_role"]
                item["awx_credential_id"] = credential["awx_credential_id"]
            else:
                # CREDENTIAL_MISSING — item will be skipped, not failed
                item["error_code"] = "CREDENTIAL_MISSING"
                item["skip_reason"] = "CREDENTIAL_MISSING"
...
class _DbFactCollectionBuilderBase(BaseCheckItemBuilder):
    """Base class for DB fact collection builders — shared asset resolution logic."""

    check_code: str = ""
    fact_category: str = "basic"  # basic, version, role
    ...
    def build(self, db, *, assets, options):
        ...
        for asset in assets:
            ...
            # MySQL is stub-only in Phase 3.3A
            ...
            # Resolve DB credential
            credential = CredentialResolverService.resolve_db_credential(db, instance_id)

            # Default database per DB type
            default_database = credential.get("default_database") if credential else None
            if not default_database:
                if db_type_code == "postgresql":
                    default_database = "postgres"
                elif db_type_code == "sqlserver":
                    default_database = "master"
                elif db_type_code == "oracle":
                    default_database = None  # Oracle uses service_name

            item_key = f"db_instance:{instance_id}:{self.check_code}:{target_host}:{target_port}"
            item = {
                "item_key": item_key,
                ...
                "is_required": False,
                "timeout_seconds": timeout_seconds,
                "asset_name": instance.instance_name or f"instance-{instance_id}",
                "db_type_code": db_type_code,
                "service_name": instance.service_name or "",
                "database_name": default_database or "",
