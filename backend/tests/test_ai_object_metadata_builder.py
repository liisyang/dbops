"""
Phase 3.6B0 C16-F3 — _AiObjectMetadataBuilder 单元测试
...
覆盖 (plan §13 Object Metadata Snapshot 矩阵的 builder 部分):
  ...
  9.  inline SQL 与磁盘模板字节级一致
 10.  _default_database 边界 (postgres alias / oracle service_name)
...
def test_sql_template_loadable_and_readonly():
    ...
    lives in pg_object_metadata sql validation — we only smoke-check here)."""
    sql = _AiObjectMetadataBuilder._load_sql_template()
    assert sql, "SQL template must not be empty"
    # 5 段 UNION ALL 段 → 至少 4 个 UNION ALL（注释里也可能提及）
    assert sql.count("UNION ALL") >= 4
    # 必须以 SELECT 开头（C7 不变式）
    body = "\n".join(
    ...
    # pg_catalog 数据源（plan §4.8 P0 修正：仅查询 catalog/information_schema）
    assert "pg_catalog" in sql
    # 5 列：object_type, schema_name, object_name, ddl_text, comment
...
def test_build_mixed_assets_returns_mixed_items():
    ...
    assert it500["check_code"] == "DB_OBJECT_METADATA"
    assert it500["business_domain"] == "ai_object_metadata"
...
def test_inline_sql_matches_disk_template():
    """rule_config.sql_text must equal the on-disk template (byte-for-byte)."""
    b = _AiObjectMetadataBuilder()
    ...
    disk_sql = _AiObjectMetadataBuilder._load_sql_template()
