...
- **状态**：⬅ 已回退（2026-07-08 bug-fix）。Commit 6 的独立凭证链路（dbops_readonly + AWX cred id=10 + profile id=12）被判定为不必要的过度设计——Oracle 961/MSSQL 963/964 三种实例均直接复用既有 collector 凭证，PG 965 不应特殊处理。binding id=15 已切回 profile_id=4（gdmms, AWX id=7）；AWX cred id=10 / profile id=12 / PG 965 dbops_readonly role 保留不删。
