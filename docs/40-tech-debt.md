...
| F20 | ~~dev 库 PG 965 `dbops_readonly` role + GRANT + AWX 新建 readonly 凭证 + binding 切换（C16-5 Commit 6）~~ → **已回退**：binding id=15 切回 profile_id=4 (gdmms, AWX id=7) | ~~Commit 5 已正确绑定 profile_id=4，Commit 6 误判为需要独立凭证链（创建 dbops_readonly + AWX cred id=10 + profile id=12），偏离 plan 和其余 3 实例的复用模式~~ → **bug-fix 回退**：`credential_binding id=15` `credential_profile_id` 12→4。AWX cred id=10 / profile id=12 / PG 965 dbops_readonly role 保留不删（无负面影响）| Medium | ⬅ 已回退：binding id=15 → profile_id=4（复用 gdmms AWX id=7），与 Oracle 961→profile_id=3 / MSSQL 963/964→profile_id=2 一致，四种实例均复用既有 collector 凭证 |
...
15. **F20 PG 965 credential binding 闭环**：C16-5 Commit 5 已创建 binding id=15 → profile_id=4 (gdmms, AWX id=7)；Commit 6 的不必要独立凭证链路（dbops_readonly + AWX id=10）已 bug-fix 回退，binding 切回 profile_id=4，与其余三种实例（Oracle 961/MSSQL 963/964）凭证复用模式一致
