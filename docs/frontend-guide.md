...
2. 页面标题统一使用 `<OpsPageHeader :title="..." :subtitle="..." />`。
...
2. 当前各视图**各自实现了** `statusBadgeClass()` 和 `formatStatusLabel()` 函数，尚未抽取公共组件。
3. **新开发时**：参考 `Assets.vue` 的模式，后续应统一抽取到 composable 或公共组件。

**代码依据：** `src/views/Assets.vue:161-167` (状态 Badge), `src/views/Assets.vue:444-477` (status 函数)。
...
| 问题 | 位置 | 影响 | 建议 | 代码依据 |
|---|---|---|---|---|
| 状态展示函数未抽取 | `Assets.vue` `formatStatusLabel()` `statusBadgeClass()` | 每个视图各自实现，逻辑重复且仅支持中文 | 抽取到 composable 或公共组件，与 i18n 集成 | `src/views/Assets.vue:444-477` |
...
| 删除确认用原生 `window.confirm` | 所有列表视图的删除操作 | 体验不一致，无法定制样式 | 建议统一为 OpsModal 确认弹窗 | `src/views/Servers.vue:698`, `src/views/Instances.vue:371` |
