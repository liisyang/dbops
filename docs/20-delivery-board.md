...
## 7. Phase 3.3A 交付物（2026-06-11 收尾）
...
- ✅ Batch 91 全资产回归：SQL Server 三类 facts 全成功；Oracle 三类 item 已创建并调度执行，失败原因为 `AUTHENTICATION_FAILED / ORA-01017`（从“未采集”收敛为“凭证内容错误”）
...
- Oracle 凭证 `cred-db-oracle-ro-prod(id=6)` 的账号口令和 service_name 需现场确认并修正（当前 ORA-01017）
