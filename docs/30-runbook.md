...
| 全资产事实采集“都成功但感觉很慢” | `db_fact_collect` 每个 item 都执行多条 Debug 命令并重复 copy collector_client，且 loop 串行 | 单 job 耗时被 debug 与串行执行放大 | 生产联调时关闭 debug 步骤、改为每 job 复制一次 collector_client，并评估 async 并行 fan-out/fan-in | `ansible-playbooks/playbooks/roles/db_fact_collect/tasks/main.yml` + `ansible-playbooks/playbooks/dbops_collector_generic.yml` |
