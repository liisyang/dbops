...
| AI Copilot - Chat | 已实现（Phase 3.6 C1-C5 + BE-bug1 修复 + Refactor 隐藏推理过程 + **C16-F2c** boundInstanceId 模式分流） | Dify chat-message app；user 维度会话/消息 CRUD + 发送（幂等 client_request_id）+ 消息历史；启动 stale cleanup；前端 Chat.vue 完整 UI（会话侧栏 + 消息流 + 输入框 + 错误兜底 409/502/503/504）；DifyService.chat_message 对 answer 字段权威剥离推理块（避免推理模型 DeepSeek R1 / o1 等混入最终答案），前端 ChatMessageBubble.displayContent computed 做显示层兜底剥离；**C16-F2c 实例绑定入口**：`route.query.boundInstanceId` 解析 → `mode='instance_sql'` → 创建/复用绑定实例 session（F2a partial unique 自动去重）→ 顶部实例上下文 header（`instance_name / db_type_code / server_ip:port`） + 「返回通用 Chat」按钮 + placeholder 切换；onSend 分流（`instance_sql`→POST /ai/sql/preview / `general`→POST /chat/sessions/{id}/messages）；ChatMessageBubble.previewLinkMeta 合并 `metadata_json`(audit_id/status) + `content` JSON(approved_sql/schema_policy_hash/reason) + rejected 分支模板（红框 + reason 显示 + 无执行按钮）；修 F2b Pydantic Literal 漏 `'sql_preview_link'` 残留导致 listMessages 500 bug | `backend/app/services/ai_chat_service.py` + `backend/app/services/dify_service.py` + `backend/app/services/ai_text.py`（剥离工具）+ `backend/app/api/ai.py:118-244` + `backend/app/schemas/ai.py` (AiChatMessageResponse.message_type Literal 修) + `frontend/src/views/ai/Chat.vue` + `frontend/src/components/ai/ChatMessageBubble.vue` + `frontend/src/views/ai/SqlPreview.vue` (boundInstanceId 重定向) + `frontend/src/views/InstanceDetail.vue` (AI 查询入口) + `frontend/src/utils/aiText.ts`（剥离工具） + `backend/tests/test_ai_chat_message_response_c16_f2c.py` |
...
| AI Copilot - SQL Preview | 已实现（Phase 3.6B1 C11-C12 + C16-F2b + **C16-5 Commit 5 三方言 capabilities 解锁 + Commit 6 dbops_readonly 最小权限闭环**） | sqlglot AST 权威层 + Dify sql-generator workflow 调用；6 层 SQL 安全防御（Layer 3 AST 落地）；双轨 SQL（generated_sql 审计追溯 vs approved_sql 权威）+ 双 SHA-256 + schema_policy_hash 强绑定；7 类异常 → HTTP 404/409/422/502/503/504；ai_sql_audit 表 24 字段/8 FK/6 CHECK/6 索引；**C16-F2b Chat 流内可调用入口**：4 步鉴权链（session ownership + chat_mode='instance_sql' + bound_instance_id 一致 + DbInstance.status='active'）+ client_request_id 幂等（partial unique 天然支持）+ 双消息写入事务（user + preview sql_preview_link + audit 三元组同事务）+ rejected 也写 preview_message 卡片（P1-3）；PreviewResult 扩 4 字段（session_id/user_message_id/preview_message_id/idempotent_replay）；5 类新异常 → HTTP 404/403/422/409 映射；**C16-5 三方言 capabilities 解锁**：`sql_supported_db_types` 从 `["POSTGRESQL"]` → `["POSTGRESQL", "ORACLE", "MSSQL"]`（MySQL 仍延后按 plan §4.8），dev 库 ai_sql_audit 卡 running 记录（id=4 instance_id=965）强制收尾为 `execution_status='timeout'` + error_message，dev 库 `ai_object_metadata_snapshot` 表 DDL 落地（4 CHECK + 5 索引），dev 库 PG instance 965 凭证绑定 `bind-db-postgresql-inst-965` (priority=10) 落到 credential_profile_id=4；**C16-5 Commit 6 最小权限闭环**：dev 库 PG 965 CREATE ROLE `dbops_readonly`（`rolsuper=f rolcreatedb=f rolcreaterole=f` 最小权限）+ 3 库（`benchdb/jemdb/postgres`）5 schema（`public/sbtest/app/oggadm`）`GRANT CONNECT + USAGE + SELECT ON ALL TABLES + 9 条 ALTER DEFAULT PRIVILEGES FOR ROLE benchuser/app/oggadm`，AWX REST 新建 cred id=10 `cred-db-postgresql-ro-prod-readonly`（username=`dbops_readonly` inputs.password=`readonly2026@readonly`，password encrypted in AWX），dbops DB `credential_profile id=12` INSERT + `credential_binding id=15` `profile_id` 4→12 UPDATE（SQL Copilot end-user preview→execute 走 dbops_readonly + AWX id=10，与 gdmms collector readonly 链路彻底隔离） | `backend/app/services/ai/ai_sql_preview_service.py` + `backend/app/services/sql_safety_service.py` + `backend/app/services/ai_chat_service.py:get_session_for_user` + `backend/app/api/ai.py:431-575` + `backend/app/config.py:200-211` (`sql_supported_db_types` 三方言) + `backend/tests/test_dify_service.py` (3 dialect tests) + `backend/db/dbops_phase3_6b0_ai_object_metadata.sql` (dev 库落地) + AWX credential id=10 (`cred-db-postgresql-ro-prod-readonly`) + dev 库 PG role `dbops_readonly`（PG 965 server-side `CREATE ROLE` + 3 库 GRANT block via `/tmp/commit6_grant_readonly.sql`） |
...
| AI Copilot - Chat 前端 boundInstanceId 模式分流（C16-F2c） | 已实现（Phase 3.6B2 C16-F2c） | 前端将 Chat 入口按 `route.query.boundInstanceId` 分流为 general / instance_sql 双模式；`InstanceDetail.vue` 「AI 查询」按钮 → `router.push({name: 'AiChat', query: {boundInstanceId: String(instance.id)}})`；`Chat.vue` 解析 query → `mode='instance_sql'` → 创建/复用绑定实例 session（F2a partial unique 自动去重）→ 顶部实例上下文 header + 「返回通用 Chat」按钮 + placeholder 切换；onSend 分流：instance_sql → POST /ai/sql/preview；general → POST /chat/sessions/{id}/messages；调用后 `loadMessages` 同步后端实际状态；`ChatMessageBubble.previewLinkMeta` 合并 `metadata_json` + `content` JSON + rejected 分支模板（红框 + reason 显示 + 无执行按钮）；`SqlPreview.vue` 检测 `boundInstanceId` → `router.replace` 重定向到 Chat.vue（commit 2 重定向逻辑：用户只看到自然语言交互和 SQL 卡片，不跳独立预览页）；修 F2b Pydantic Literal 漏 `'sql_preview_link'` 残留 bug（listMessages 500） | `frontend/src/views/InstanceDetail.vue` (`gotoAiChat` + 「AI 查询」按钮) + `frontend/src/views/ai/Chat.vue` (`parseBoundInstanceId` + `loadBoundInstanceContext` + `createBoundSession` + `exitBoundMode` + `activeSessionIsInstanceSql` + `inputPlaceholder` + onSend 分流) + `frontend/src/components/ai/ChatMessageBubble.vue` (`previewLinkMeta` 重写 + rejected 分支) + `frontend/src/views/ai/SqlPreview.vue` (`parseBoundInstanceIdFromQuery` + onMounted 重定向) + `frontend/src/types/ai.ts` (AiChatSessionCreateRequest 3 字段 + AiChatSession 2 字段 + AiSqlPreviewRequest 必填 2 字段 + AiSqlPreviewResponse 4 字段 + AiSqlPreviewLinkMetadata reason) + `frontend/src/api/ai.ts` (createSession 透传 mode/bound_instance_id) + `backend/app/schemas/ai.py` (AiChatMessageResponse.message_type Literal 修) + `backend/tests/test_ai_chat_message_response_c16_f2c.py` (3 回归测试) |

| AI Copilot - Chat 跨入口一致性回归（C16-F17） | 已实现（Phase 3.6B2 C16-F17） | F2a/F2b/F2c 测试已覆盖跨入口一致性后端逻辑（partial unique 复用 + 4 步鉴权 + 幂等 + 双消息 + Pydantic Literal 5 值）；F17 在 ChatMessageBubble.spec.ts 追加 6 cases 回归 previewLinkMeta 5 message_type 解析分支（passed+approved_sql / passed 缺 SQL / rejected+reason / rejected 缺 reason / 非 JSON content 脏数据 / chat 走普通气泡），确保 form 模式 `source_page='sql_preview_legacy'` 与 Chat 模式 `source_page='instance_detail'` 写入的 preview_message 经 listMessages 回到前端都被正确解析与渲染 | `frontend/src/components/ai/ChatMessageBubble.spec.ts`（+168 行：1 helper + 6 cases） |
