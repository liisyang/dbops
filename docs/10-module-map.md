...
| AI Copilot - SQL Preview | 已实现（Phase 3.6B1 C11-C12 + C16-F2b + **C16-5 Commit 5 三方言 capabilities 解锁**） | sqlglot AST 权威层 + Dify sql-generator workflow 调用；6 层 SQL 安全防御（Layer 3 AST 落地）；双轨 SQL（generated_sql 审计追溯 vs approved_sql 权威）+ 双 SHA-256 + schema_policy_hash 强绑定；7 类异常 → HTTP 404/409/422/502/503/504；ai_sql_audit 表 24 字段/8 FK/6 CHECK/6 索引；**C16-F2b Chat 流内可调用入口**：4 步鉴权链（session ownership + chat_mode='instance_sql' + bound_instance_id 一致 + DbInstance.status='active'）+ client_request_id 幂等（partial unique 天然支持）+ 双消息写入事务（user + preview sql_preview_link + audit 三元组同事务）+ rejected 也写 preview_message 卡片（P1-3）；PreviewResult 扩 4 字段（session_id/user_message_id/preview_message_id/idempotent_replay）；5 类新异常 → HTTP 404/403/422/409 映射；**C16-5 三方言 capabilities 解锁**：`sql_supported_db_types` 从 `["POSTGRESQL"]` → `["POSTGRESQL", "ORACLE", "MSSQL"]`（MySQL 仍延后按 plan §4.8），dev 库 ai_sql_audit 卡 running 记录（id=4 instance_id=965）强制收尾为 `execution_status='timeout'` + error_message，dev 库 `ai_object_metadata_snapshot` 表 DDL 落地（4 CHECK + 5 索引），dev 库 PG instance 965 凭证绑定 `bind-db-postgresql-inst-965` (priority=10) 复用既有 gdmms 凭证（credential_profile_id=4 → AWX cred id=7），与 Oracle 961/MSSQL 963/964 三种实例的凭证复用模式保持一致 | `backend/app/services/ai/ai_sql_preview_service.py` + `backend/app/services/sql_safety_service.py` + `backend/app/services/ai_chat_service.py:get_session_for_user` + `backend/app/api/ai.py:431-575` + `backend/app/config.py:200-211` (`sql_supported_db_types` 三方言) + `backend/tests/test_dify_service.py` (3 dialect tests) + `backend/db/dbops_phase3_6b0_ai_object_metadata.sql` (dev 库落地) |
